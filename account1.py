...
from database import (
    init_db, get_dashboard_stats, add_transaction, get_transactions,
    add_invoice, get_invoices, mark_invoice_paid, delete_transaction, delete_invoice,
...
if page == "Dashboard":
    ...
    with col4:
        st.metric(
            ...
            delta=f"{stats['overdue_count']} overdue" if stats['overdue_count'] > 0 else None,
...
# INCOME
    ...
    with tab1:
        ...
        with st.form("income_form"):
            ...
            if submitted:
                if amount > 0:
                    add_transaction(date_input.isoformat(), "income", category, amount, description)
...
# EXPENSES
    ...
    with tab1:
        ...
        with st.form("expense_form"):
            ...
            if submitted:
                if amount > 0:
                    add_transaction(date_input.isoformat(), "expense", category, amount, description)
...
# INVOICES
    ...
    with tab1:
        ...
        with st.form("invoice_form"):
            col1, col2 = st.columns(2)
            with col1:
                invoice_num = st.text_input("Invoice Number", value=f"INV-{datetime.now().strftime('%Y%m%d')}-001")
                client_name = st.text_input("Client Name")
                amount = st.number_input("Amount ($)", min_value=0.0, step=0.01, format="%.2f")
            with col2:
                issue_date = st.date_input("Issue Date", date.today())
                due_date = st.date_input("Due Date", date.today())
            description = st.text_area("Description/Notes", placeholder="Services rendered...")
            ...
            if submitted:
                if client_name and amount > 0:
                    try:
                        add_invoice(invoice_num, client_name, amount, 
                                   issue_date.isoformat(), due_date.isoformat(), description)
                        st.success(f"✅ Invoice {invoice_num} created for ${amount:,.2f}!")
                        st.rerun()
    ...
    with tab2:
        ...
        if invoices:
            ...
            with col1:
                filter_status = st.selectbox("Filter", ["All", "Paid", "Unpaid", "Overdue"])
            ...
            if unpaid_invs:
                ...
                with col1:
                    inv_to_pay = st.selectbox(
                        "Select invoice",
                        options=unpaid_invs,
                        format_func=lambda x: f"{x.invoice_number} - {x.client_name} (${x.amount:,.2f})"
                    )
                with col2:
                    st.markdown("<br>", unsafe_allow_html=True)
                    if st.button("✅ Mark Paid", use_container_width=True):
                        mark_invoice_paid(inv_to_pay.id)
                        st.success(f"Invoice {inv_to_pay.invoice_number} marked as paid!")
                        st.rerun()
            else:
...
# REPORTS
    ...
    if report_type == "Profit & Loss Summary":
        ...
        if transactions:
            ...
            fig = go.Figure()
            fig.add_trace(go.Bar(x=summary['month'], y=summary['income'], name='Revenue', marker_color='#10B981'))
            fig.add_trace(go.Bar(x=summary['month'], y=summary['expense'], name='Expenses', marker_color='#EF4444'))
            fig.add_trace(go.Scatter(x=summary['month'], y=summary['profit'], name='Profit', line=dict(color='#3B82F6', width=3)))
        ...
        with col1:
            ...
            income = get_transactions(type_="income")
            if income:
                df = pd.DataFrame([{'Category': t.category, 'Amount': t.amount} for t in income])
                cat_sum = df.groupby('Category')['Amount'].sum().reset_index()
        ...
        with col2:
            ...
            expenses = get_transactions(type_="expense")
            if expenses:
                df = pd.DataFrame([{'Category': t.category, 'Amount': t.amount} for t in expenses])
                cat_sum = df.groupby('Category')['Amount'].sum().reset_index()
                fig = px.pie(cat_sum, values='Amount', names='Category', hole=0.4)
                st.plotly_chart(fig, use_container_width=True)
            else:
        ...
        if invoices:
            ...
            with col2:
                fig = px.bar(df_aging, x='Aging Bucket', y='Amount',
                           color='Aging Bucket',
                           color_discrete_map={
